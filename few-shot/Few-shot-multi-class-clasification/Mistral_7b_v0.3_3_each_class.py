...
REASONING_TEMPLATES = {
    1: "The user employs sarcasm/humour to make a generalisation. While not overtly hateful, it relies on a subtle stereotype, which fits Severity 1.",
    2: "This comment contains a direct insult regarding the region's people/infrastructure without any sarcasm. It is explicitly negative but does not use slurs, fitting Severity 2.",
    3: "This comment uses aggressive/dehumanising language and slurs. It goes beyond criticism into hate speech, fitting Severity 3."
}
...
def create_prompt_and_test_sets(df: pd.DataFrame, examples_per_class: int) -> tuple[str, pd.DataFrame]:
    print(f"\nSelecting {examples_per_class} examples per class for the CoT support set.")
    df_few_shot = pd.DataFrame()
    
    for label in [1, 2, 3]:
        df_class = df[df['level-2'] == label]

        short_candidates = df_class[df_class['comment'].str.len() < 300]
    ...
    examples_str = "### Reference Examples\n\n"
    for comment, label in all_examples:
        reasoning = REASONING_TEMPLATES.get(label, "Matches definition.")
        examples_str += (
            f"Comment: \"{comment}\"\n"
...
def parse_response(response: str) -> tuple[str, int]:
    ...
    if "Classification:" in cleaned_response:
        parts = cleaned_response.split("Classification:")
...
def main():
    ...
    print("\nClassification Report (CoT - Classes 1, 2, 3):")
    report = classification_report(y_true, y_pred, target_names=['SEV 1', 'SEV 2', 'SEV 3'], labels=[1,2,3], zero_division=0)
    print(report)
    ...
    plt.title('Confusion Matrix (Mistral CoT)')
